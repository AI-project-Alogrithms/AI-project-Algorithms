...
def initialize_weights(m):
    if hasattr(m, 'weight') and m.weight.dim() > 1: # hasattr -> hasattribute
        nn.init.kaiming_uniform(m.weight.data) # initialization 방법
...
model.apply(initialize_weights) # apply가 뭐지? 어케 적용되는 것?
optimizer = Adam(params=model.parameters(),
                 lr=init_lr,
                 weight_decay=weight_decay,
                 eps=adam_eps)

# factor와 patience의 의미
        ...
        for i, batch in enumerate(iterator):
            ...
            for j in range(batch_size):
                try:
                    ...
                    output_words = output[j].max(dim=1)[1] # 의미?
